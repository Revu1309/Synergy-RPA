...
def seed_default_access_config() -> None:
    ...
    try:
        ...
        # Ensure admin gets all pages by default.
...
def is_admin_user(username: str) -> bool:
    ...
    try:
        cur.execute(
            "SELECT is_admin FROM app_users WHERE username = %s AND is_active = TRUE",
            (username,),
        )
        row = cur.fetchone()
...
def get_modules_with_pages() -> List[Dict]:
    ...
    try:
        ...
        for row in rows:
            ...
            modules[module_key]["pages"].append(
                {
...
def get_user_menu_modules(username: str) -> List[Dict]:
    ...
    for mod in modules:
        pages = [
            p
            for p in mod["pages"]
            if p["is_menu_visible"] and p["page_key"] in allowed
        ]
        if pages:
            result.append(
                {
...
def get_first_allowed_route(username: str) -> str:
    ...
    try:
        cur.execute(
            ...
            WHERE upp.username = %s
              AND upp.is_allowed = TRUE
...
def user_can_access_path(username: str, path: str) -> bool:
    ...
    try:
        cur.execute(
            "SELECT page_key FROM app_pages WHERE route_path = %s AND is_active = TRUE",
            (normalized,),
        )
        row = cur.fetchone()
        if not row:
            # Routes not configured in page table are considered unrestricted.
            return True
        page_key = row[0]
        cur.execute(
            """
            ...
            WHERE username = %s AND page_key = %s
            """,
            (username, page_key),
        )
        perm = cur.fetchone()
